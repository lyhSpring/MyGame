...
public class CarController : MonoBehaviour
{
    ...
    //完成圈数最少检查点
    private int minCheckPoints = 100;
    private int totalCircleNumber = 1;//总圈数
    ...
    void OnTriggerEnter(Collider Trigger)
    {
        ...
        if (minCheckPoints <= CheckPoints.Count)
        {
            ...
            if (currentCircleNumber > totalCircleNumber)
            {
                ...
                play_Complete.text = "游戏结束";
                qiangti.SetActive(true);
           ...
           //----------------------------------------------------------------------------
            plan_text.text = "当前圈数：" + currentCircleNumber.ToString();
            //----------------------------------------------------------------------------
            //清空检测点
            CheckPoints.Clear();
        }
    }
    public  Text plan_text;

    public Text play_Complete;

    public Wheel scene;

    public GameObject qiangti;
    #endregion
}
